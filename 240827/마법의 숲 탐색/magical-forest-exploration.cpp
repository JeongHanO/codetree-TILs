...
void move(int center_x, int center_y) { // 정령 x, y에서 바닥까지 이동
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < 4; i++) {
			int nx = x + dx[i];
			int ny = y + dy[i];
			if (nx <= 1 || ny <= 1 || nx > R+1 || ny > C+1 || board[nx][ny] == 0 || visited[nx][ny]) continue;
...
void move_gol(int num) {
	...
	while (1) { // 더이상 움직일 수 없을 때까지 이동
		...
		bool flag = false;
		// 왼쪽 아래로 갈 수 있음
		if (y-2 > 0 && depth+2 <= R+1 && board[depth][y - 2] == 0 && board[depth - 1][y - 1] == 0 && board[depth + 1][y - 1] == 0 && board[depth+2][y-1] == 0 && board[depth+1][y-2] == 0 && board[depth+1][y] == 0) {
			y--;
			dir = (dir - 1 + 4) % 4;
			flag = true;
		}
		// 오른쪽 아래로 갈 수 있음
		else if (y+2 <= C && depth + 2 <= R+1 && board[depth][y + 2] == 0 && board[depth - 1][y + 1] == 0 && board[depth + 1][y + 1] == 0 && board[depth + 2][y + 1] == 0 && board[depth + 1][y] == 0 && board[depth + 1][y+2] == 0) {
			y++;
			dir = (dir + 1) % 4;
			flag = true;
		}
		// @@@@@@@@@ depth가 0에 멈춰있을 때, depth-1의 범위 계산 && 현재 depth(중심)이 무조건 0부터 시작함. 그런데 사실은 depth+1 == 0인 위치부터 시작해야함
